...
powershell -NoProfile -Command "$src = [string]::Join([Environment]::NewLine, @('import \"../examples/math.sp\";','','spear launch() {','    let bad = ;','}')); $src | & '.\\build\\spearc.exe' --check-stdin 'build\\regression_import_line.sp' 2>&1 | Set-Content 'build\\regression_import_line.log'; exit 0"
findstr /C:"spearc error [line 4:" build\regression_import_line.log >NUL || exit /b 1

powershell -NoProfile -Command "$lines = @('view card(text title) {','    return markup(\"section\") {','        markup(\"h1\") {','            escape(title);','        };','        row {','            action(\"/start\", \"Open\");','            action(\"#docs\", \"Docs\");','        };','    };','}','', 'spear launch() {','    text html = page(\"Demo\") {','        column {','            card(\"Declarative UI\");','        };','    };','    write(\"build/ui.html\", html);','}'); [System.IO.File]::WriteAllText('build\\regression_ui.sp', [string]::Join([Environment]::NewLine, $lines))" || exit /b 1
build\spearc.exe --check build\regression_ui.sp >NUL 2>&1 || exit /b 1

del /Q build\regression_loop.sp build\regression_loop.c build\regression_import_line.log build\regression_ui.sp 2>NUL
exit /b 0
